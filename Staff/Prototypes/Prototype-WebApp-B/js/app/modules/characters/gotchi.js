define(['Phaser', 'jquery', 'modules/helpers/logger', 'modules/models/gotchiCollection'], function(Phaser, $, Logger, gotchiCollection) {
    ...
    function Gotchi(gotchiModel) {
        this._gamePtr = null;
        this._gotchi = null;
        this._model = gotchiModel;
    }

    Gotchi.prototype.preload = function(game) {
        this._gamePtr = game;
        this._gamePtr.load.spritesheet(this._model.get('name'), this._model.get('spriteSheet.imagePath'),
            this._model.get('spriteSheet.boxSize.width'), this._model.get('spriteSheet.boxSize.height'),
            this._model.get('spriteSheet.frames'));
    };
    ...
    Gotchi.prototype.create = function() {
        ...
        this._gotchi = this._gamePtr.add.sprite(this._model.get('spriteSheet.position.x'),
            this._model.get('spriteSheet.position.y'), this._model.get('name'));
    ...
    Gotchi.prototype._dispacheStates = function(statesActions) {
        ...
        for (var state in states) {
            ...
            if (timeDiff >= states[state].time.interval) {
                Logger.MSG_DESP('gotchi is in ' + state + ' state. Diff : ' + timeDiff + 'ms' + ' Elapsed : ' + this._gamePtr.time.totalElapsedSeconds() + 'secs');
    ...
    return {
        create: function(gotchiModel) {
            return new Gotchi(gotchiModel);
        },
        this: Gotchi
